...
public class CloudProviderDetector {
    ...
    private static CloudProvider detect() {
        if (isGke()) {
    ...
    private static boolean isGke() {
        try {
            HttpURLConnection conn = openConnection(METADATA_URL + "/computeMetadata/v1/");
    ...
    private static boolean isEks() {
        try {
            HttpURLConnection conn = openConnection(METADATA_URL + "/latest/meta-data/");
            int code = conn.getResponseCode();
    ...
    private static boolean isAks() {
        try {
            HttpURLConnection conn = openConnection(METADATA_URL + "/metadata/instance?api-version=2021-02-01");
